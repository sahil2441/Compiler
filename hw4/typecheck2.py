...
def checkAllArgSame(currentArgList):
    if len(currentArgList) < 1 :
        return True
    firstArg = ''
    for key in currentArgList:
        firstArg = key
        break
    for key in currentArgList:
        if str(firstArg)!= str(key):
            return False
    return True

def getParentType(currentArgList):
    result = []
    # for arg in currentArgList:
        # result =
    return result


def checkParentListExists(parentType, argumentTypeList):
    for args in argumentTypeList:
        for i in range(0, len(args)):
            if parentType[i] != args[i]:
                return False
    return True


def containsSameArg(sameArgList, currentArgList):
    for argList in sameArgList:
        for i in range(0,len(currentArgList)):
            if currentArgList[i] == argList[i] and i == len(currentArgList):
                print "currentArgList[i]: ", currentArgList[i]
                print "argList[i]:  ",argList[i]
                return True
            elif currentArgList[i] != argList[i]:
                break
    return False



def checkRepConstMeth(classtable):
    # Check repetitive methods/ constructors
    for cid in classtable:
        c = classtable[cid]
        # print c
        # check for individual class
        # maintain max number of arguments
        maxArguments=0
        for cons in c.constructors:
            numberOfArgs = len(cons.vars.get_params())
            if numberOfArgs > maxArguments:
                maxArguments = numberOfArgs
        # print "maxArguments: ", maxArguments

        for i in range (2, maxArguments+1):
            print "i: ", i
            global argumentTypeList
            argumentTypeList = []
            sameArgList = []
            ...
            for cons in c.constructors:
                numberOfArgs = len(cons.vars.get_params())
                if i == numberOfArgs:
                    # Add all constructor argument type in list
                    currentArgList = cons.vars.vars[0]
                    # check if all arg type are same e.g int, int, int
                    # if yes, add them to a list
                    if(checkAllArgSame(currentArgList)):
                        if containsSameArg(sameArgList, currentArgList):
                            print "Error! Can't have a constructor with same argument types."
                        else:
                            sameArgList.__add__(currentArgList[0])

                    # if all arguments type is not same e.g. int,float
                    # find the parent type of int i.e. float and store float, float in a map
                    else:
                        pass
                        # parentType = getParentType(currentArgList)
                        # if checkParentListExists(parentType, argumentTypeList):
                        #     print "Error! Can't have a similar constructor."
                        #     break
                        # else:
                        #     argumentTypeList.__add__(parentType)
